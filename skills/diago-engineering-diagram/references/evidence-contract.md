...
- `kind`: `fact`, `assumption`, or `recommendation`;
- `source`: a file path, URL, command result, or explicit user statement;
- `confidence`: `high`, `medium`, or `low`;
- `scope`: the environment, version, branch, or time window.

If a source is stale or environment-specific, show that limitation in the diagram. If evidence conflicts, render the conflict as an open question rather than choosing silently.
