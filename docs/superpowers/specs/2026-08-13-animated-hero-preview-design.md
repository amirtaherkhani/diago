...
Success means a visitor can understand, within one animation cycle, that Diago can visualize system structure, domain relationships, delivery chronology, and responsibility or control placement without losing evidence status or reviewability.

## Selected Direction

Use one accessible, auto-playing workbench with four views:

1. **Architecture** — checkout request boundaries and dependencies.
2. **Data model** — order, payment, and idempotency relationships.
3. **Timeline** — an evidence-aware payment migration sequence.
4. **Layers** — where checkout responsibilities and controls are enforced.

The preview advances every five seconds. A visible pause/play control gives the visitor direct control. Selecting a tab changes the view immediately and restarts the interval from that selection.
...
- Auto-play advances Architecture → Data model → Timeline → Layers → Architecture.
...
- Left and Right Arrow move between tabs. Home selects Architecture; End selects Layers. Focus follows the selected tab.
...
The footer retains verified, assumption, current, and proposed semantics across all four views. Each panel uses compact SVG or semantic HTML drawn from the existing Diago visual language; no screenshot or rendered bitmap is embedded.

## Implementation Boundaries

- `docs/index.html` owns the four tab controls, four preview panels, accessible labels, filenames, and pause/play control.
...
- At narrow widths, the view switcher scrolls horizontally inside its own region without causing page-level overflow.
...
- Fresh visual captures at 375, 768, and 1280 CSS pixels showing every settled view plus the pause/play and active-progress states.
...
- GitHub Pages deployment succeeds and the live page contains the four-view preview.
