...
<body class="quality-${escapeHtml(model.quality ?? 'showcase')}">
  <main>
    ...
    <section class="canvas" aria-label="${title}">
      <svg class="diagram-view diagram-desktop" role="img" aria-labelledby="${diagramId}-title ${diagramId}-desc" viewBox="0 0 ${model.width} ${model.height}" tabindex="0">
      ...
      <svg class="diagram-view diagram-mobile" role="img" aria-labelledby="${diagramId}-mobile-title ${diagramId}-mobile-desc" viewBox="0 0 ${mobile.width} ${mobile.height}" tabindex="0">
        <title id="${diagramId}-mobile-title">${title}</title>
        <desc id="${diagramId}-mobile-desc">${description}</desc>
        <defs><marker id="arrow-mobile" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#6bbcff"/></marker></defs>
        ${mobile.svg}
      </svg>
