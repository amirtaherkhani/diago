...
import { mobileDataModel } from './mobile/data-model.mjs';
...
function relationshipSvg(relationship, positions) {
  ...
  const interveningEntity = sameRow && [...positions.entries()].some(([id, box]) => (
  ...
  const route = interveningEntity ? 'outer' : 'direct';
  let path;
  let labelX = (startX + endX) / 2;
  let labelY = ((startY + endY) / 2) + 22;

  if (interveningEntity) {
    const startCorridorX = startX + (forward ? 24 : -24);
    const endCorridorX = endX + (forward ? -24 : 24);
    const routeY = Math.max(24, Math.min(from.y, to.y) - 32);
    path = `M ${startX} ${startY} L ${startCorridorX} ${startY} L ${startCorridorX} ${routeY} L ${endCorridorX} ${routeY} L ${endCorridorX} ${endY} L ${endX} ${endY}`;
    labelX = (startCorridorX + endCorridorX) / 2;
    labelY = routeY - 8;
  } else if (sameRow) {
  ...
  return `<g data-route="${route}" aria-label="${escapeHtml(relationship.label)}">
    <path class="relationship status-${escapeHtml(relationship.status)}" d="${path}" marker-end="url(#arrow)"/>
    ...
    <text class="relationship-label" x="${endX + (forward ? -10 : 10)}" y="${endY + (interveningEntity ? 43 : -9)}" text-anchor="${forward ? 'end' : 'start'}">${escapeHtml(relationship.toCardinality)}</text>
...
export function renderDataModel(diagram, quality = 'showcase') {
  ...
  const mobile = mobileDataModel(diagram);
